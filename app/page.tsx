...
export default function ProspectingTool() {
  ...
  const [editingProspectData, setEditingProspectData] = useState<Partial<Prospect>>({});
  ...
  useEffect(() => {
    if (isAuthenticated) {
      (async () => {
        ...
        else console.error("Error loading prospects:", error);
      })();
    }
  }, [isAuthenticated]);
  ...
  const toggleSelectProspect = (id: number) => {
    setSelectedProspects(
      selectedProspects.includes(id)
        ? selectedProspects.filter((sid) => sid !== id)
        : [...selectedProspects, id]
    );
  };

  const exportToCSV = () => {
    const headers = "ID,Name,Email,Company,Status,Source,Date Added,Notes";
    ...
    return (
      <div className="min-h-screen flex flex-col justify-center items-center bg-gray-50 dark:bg-gray-900 p-4">
        <form onSubmit={handlePasswordSubmit} className="bg-white dark:bg-gray-800 p-6 rounded shadow-md w-full max-w-sm">
          ...
          />
          <button type="submit" className="w-full py-2 px-4 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded hover:from-purple-600 hover:to-indigo-700 transition-all transform hover:scale-105">
            Unlock
          </button>
        </form>
      </div>
      ...
      <div className={`min-h-screen p-8 ${darkMode ? "bg-gray-900 text-gray-50" : "bg-gray-50 text-gray-900"}`}>
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setIsAddingProspect(!isAddingProspect)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg hover:from-purple-600 hover:to-indigo-700 transition-all transform hover:scale-105 shadow-md">
              <FiUserPlus /> {isAddingProspect ? "Cancel" : "Add Prospect"}
            </button>
            <button onClick={() => setDarkMode(!darkMode)} className="p-2 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors" aria-label="Toggle dark mode" title="Toggle dark mode">
        ...
        {/* Message */}
        {message && (
          <div className="mb-4 p-3 rounded border border-purple-500 bg-purple-100 text-purple-800">{message}</div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 p-3 border border-gray-300 rounded-lg dark:bg-gray-800 dark:border-gray-600 dark:text-gray-50 focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all"
                  value={searchQuery}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-gray-900 dark:text-gray-50 font-medium">
                          {isEditing ? (
                            <input
                              type="text"
                              value={editingProspectData.name ?? prospect.name}
                              onChange={(e) =>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">
                          {isEditing ? (
                            <input
                              type="email"
                              value={editingProspectData.email ?? prospect.email}
                              onChange={(e) =>
                                setEditingProspectData({
                                  ...editingProspectData,
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300 font-medium">
                          {isEditing ? (
                            <input
                              type="text"
                              value={editingProspectData.company ?? prospect.company}
                              onChange={(e) =>
                                setEditingProspectData({
                                  ...editingProspectData,
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">{prospect.source}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-300">{prospect.date_added}</td>
                        <td className="px-6 py-4 whitespace-nowrap flex gap-2">
                          {isEditing ? (
                            <>
                              <button
                                onClick={async () => {
                                  if (!editingProspectData.name || !editingProspectData.email || !editingProspectData.company) {
                                    alert("Name, Email, and Company cannot be empty.");
                                    return;
                                  }
                                  ...
                                  if (!error) {
                                    setProspects((prev) =>
                                      prev.map((p) =>
                                        p.id === prospect.id ? { ...p, ...editingProspectData } : p
                                      )
                                    );
              ...
              <form onSubmit={handleAddProspect} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewProspect({ ...newProspect, status: e.target.value as Prospect["status"] })}
                  >
                    <option value="New">New</option>
                    <option value="Contacted">Contacted</option>
                    <option value="Converted">Converted</option>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewProspect({ ...newProspect, source: e.target.value as Prospect["source"] })}
                  >
                    <option value="LinkedIn">LinkedIn</option>
                    <option value="Email">Email</option>
                    <option value="Twitter">Twitter</option>
